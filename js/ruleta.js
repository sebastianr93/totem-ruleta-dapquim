...
function dibujarRuleta() {
  ...
  for (let i = 0; i < numSectores; i++) {
    ...
    ctx.fillStyle = ["#bdc000ff", "#8A2BE2", "#0000FF", "#126b00ff", "#ce0000ff", "#f8a100ff"][i % 6];
    ctx.fill();
    ctx.stroke();
...
function girarRuleta() {
  ...
  // --- cálculo en grados (coincide con tu uso de CSS rotate)
  ...
  // TARGET: donde está la flecha fija (en tu caso, 12 en punto)
  // 0 = derecha (3h), 90 = abajo (6h), 180 = izquierda (9h), 270 = arriba (12h)
  const targetAngle = 270;

  // delta normalizado 0..359
  const delta = ((targetAngle - anguloCentro) % 360 + 360) % 360;
  const anguloFinal = girosCompletos * 360 + delta;

  anguloActual = anguloFinal;

  // asegurar que la rotación use el centro del canvas
  canvas.style.transformOrigin = "50% 50%";
  canvas.style.transition = "transform 3s cubic-bezier(.25,.8,.25,1)";

  // fuerza reflow y aplica la rotación
  canvas.offsetWidth;
  canvas.style.transform = `rotate(${anguloActual}deg)`;

  // (opcional) logs para depurar
  console.log("segmentos:", resultado.totalSegmentos, "indice:", resultado.indiceSegmento);
  console.log("anguloCentro:", anguloCentro.toFixed(2), "target:", targetAngle, "delta:", delta.toFixed(2), "anguloFinal:", anguloFinal);

  // listener de fin de transición
  canvas.addEventListener("transitionend", function handler() {
    canvas.removeEventListener("transitionend", handler);

    mostrarResultado(resultado.premio, resultado.segmento);

    // Normalizar ángulo para evitar acumulación grande
...
function crearMenuControl() {
  ...
  btnMenu.style.border = "2px solid #3007e71f";
